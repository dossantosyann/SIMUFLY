...
function App() {
  ...
  const handleGenerateFile = async () => {
    try {
      // Créer un FormData pour envoyer les données comme un formulaire
      const formData = new FormData();
      formData.append('autoMovementParam', autoMovementParam);

      const response = await axios.post('http://localhost:8080/generate_file', formData, {
        responseType: 'blob', // Important pour bien récupérer le fichier binaire
        headers: {
          'Content-Type': 'multipart/form-data', // Important pour que le serveur comprenne qu'il s'agit d'un formulaire
        },
      });
  ...
  return (
    <>
      ...
      {/* Champ pour le nom du chantier */}
      <div className="projectName-zone fade-in-up" style={{ marginBottom: '30px', marginTop: '20px' }}>
